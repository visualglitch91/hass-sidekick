...
export default class Sidekick {
  ...
  service(domainAndService: string, data: any) {
    ...
    return callService(this._connection, domain, service, data).then(
      () => {},
  ...
  private async _getUniqueId(entityId: string) {
    const registry = (await this._connection.sendMessagePromise({
  ...
  private _createHelper({
    ...
    const promise = (async () => {
      ...
      if (!this._entities[fullEntityId]) {
        ...
        await queue.add(() =>
          this._connection.sendMessagePromise({
            ...extra,
            type: `${domain}/create`,
